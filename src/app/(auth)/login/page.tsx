...
import { api } from '@/lib/pi-client'; // 🚀 Import Axios client
...
export default function LoginPage() {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      // 🚀 Hits the real backend endpoint
      const response = await api.post('/v1/auth/verify-otp', { email, otpCode });
      const { token } = response.data;

      // Save the real JWT token generated by Fastify
      document.cookie = `@ProjectHealth:token=${token}; path=/; max-age=86400; SameSite=Lax`;
      
      // Redirect to dashboard
      window.location.href = '/dashboard';
    } catch (err: any) {
      setError(err.response?.data?.message || "Server offline or incorrect code.");
    }
  };
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-slate-50/50 p-4">
      <Card className="w-full max-w-md rounded-2xl border-slate-100 shadow-xl bg-white">
        ...
        <CardContent>
          ...
          {step === 'email' ? (
            /* Step 1: Email Request Interface */
